resource "null_resource" "app" {
  ...
  provisioner "remote-exec" {
    connection {
      ...
      inline = [
          "ansible-pull -U https://github.com/b52-clouddevops/ansible.git -e MONGO_URL= -e COMPONENT=${var.COMPONENT} -e DB_PASSWORD=RoboShop@1 -e ENV=${var.ENV} -e APP_VERSION=${var.APP_VERSION} -e ENV=dev robot-pull.yml"
        ]
    }
